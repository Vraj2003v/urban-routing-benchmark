"""
capture_plans.py â€” Analyze benchmark results: plan regression, latency stats, index usage.
...
def parse_args():
    """Parse command-line arguments for the plan capture script.
    
    Returns:
        argparse.Namespace: Parsed arguments including input CSV path.
        
    Notes:
        Input CSV must contain columns: ts, system, src, tgt, latency_ms, plan_hash, error.
        Generated by workload/run_benchmark.py during benchmark execution.
    """
...
def write_report(df, output_dir):
    ...
    print(f"\n[Analysis] Plan regression CSV â†’ {plan_path}")

    # Full regression report text
    report_path = output_dir / "regression_report.txt"
    with open(report_path, "w") as f:
        f.write("Urban Routing Benchmark â€” Regression Report\n")
        f.write("=" * 60 + "\n\n")
        for system in ["postgres", "neo4j"]:
            sub = df[df["system"] == system]["latency_ms"]
    ...
    print(f"[Analysis] Text report â†’ {report_path}")
...
def main():
    ...
    print("Next step: python dashboard/app.py  â†’  http://localhost:8050")
